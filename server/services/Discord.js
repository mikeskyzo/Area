...
{
	const token = await global.findInDbAsync(global.CollectionToken, {user_id : user_id, service : global.Services.Discord});
	if (!token)
		return false;
	return true;
};
...
exports.redirect_auth = async function (req, json)
{
	...
	const token = await global.findSomeInDbAsync(global.CollectionToken, {user_id : req.body.user_id, service : global.Services.Discord});
	if (token)
...
{
	const token = await global.findInDbAsync(global.CollectionToken, {user_id : area.user_id, service : global.Services.Discord});
	if (!token)
	...
	const url = `https://discordapp.com/api/webhooks/${token.webhook_id}/${token.webhook_token}`;
...
exports.send_message_check_args = function(json)
{
	...
	const avatar = global.getParam(json.reaction.params, "avatar");
